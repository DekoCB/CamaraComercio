...
- `tests/Feature/` — pruebas de extremo a extremo por HTTP (login con límite de intentos, RBAC con 403 real, CRUD de asociados con validación, recuperación de contraseña de un solo uso, administración de usuarios/roles/módulos, generación/edición/anulación de facturas, pagos, cartera y reportes con exportación, importación desde Excel, configuración de perfil propio incluida foto/contraseña, documentos escaneados por asociado y control de beneficios), usando `RefreshDatabase` sobre **SQLite en memoria** (nunca toca la base de datos MySQL de desarrollo). 197 tests en total.
...
- **Parte 1 de la retroalimentación del cliente (17 sep 2026):** ✅ Completado — 4 roles nuevos (Gerencia, Logística, Marketing, Gestión de Asociados), N° de operación en pagos, editar/anular facturas (sin borrado físico, mismo patrón que anular pagos), reporte de cobranza por rango de fechas, reporte de productividad por cobrador, y exportación Excel/PDF de Cartera. Detalle en `docs/PROJECT_ANALYSIS.md` sección 10.26.
- **Parte 2 de la retroalimentación del cliente (18 sep 2026):** ✅ Completado lo accionable ahora — documentación escaneada por asociado (título de propiedad, licencia, convenios) con conversión automática a PDF; logo/foto del asociado ampliado a 10 MB; control de beneficios (ej. uso anual del auditorio) con cupo por año y aviso al agotarse. El portal de autoservicio del asociado (el cliente mismo lo enmarcó como "a futuro"), la integración de asientos contables ("contasis") y poblar Reportes con el Excel real de deudas quedan pendientes de que el cliente envíe esa información. Detalle en `docs/PROJECT_ANALYSIS.md` sección 10.27.
