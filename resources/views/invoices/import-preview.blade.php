...
@section('title', 'Vista previa de importación de facturas')
...
@section('content')
    <x-page-header title="Vista previa de importación" subtitle="Revisa los datos antes de confirmar. Ninguna fila se guarda todavía.">
        <x-slot:actions>
            <form method="POST" action="{{ route('invoices.import.cancel') }}">
                @csrf
                <button type="submit" class="btn btn-secondary btn-sm">Cancelar</button>
            </form>
            <form method="POST" action="{{ route('invoices.import.confirm') }}"
                  data-confirm="¿Confirma importar {{ $validCount }} factura(s)? Esta acción no se puede deshacer."
    ...
    <div class="d-flex gap-2 mb-3">
        <span class="badge badge-success">{{ $validCount }} listas para importar</span>
        @if ($errorCount > 0)
                ...
                <table class="data-table">
                    ...
                    @foreach ($rows as $row)
                        <tr class="{{ $row['errors'] !== [] ? 'row-danger' : '' }}">
                            ...
                            <td class="cell-muted">{{ $row['issue_date'] ? format_date($row['issue_date']) : '-' }}</td>
                            <td class="cell-muted">{{ $row['due_date'] ? format_date($row['due_date']) : '-' }}</td>
                            <td>
                                @if ($row['errors'] === [])
