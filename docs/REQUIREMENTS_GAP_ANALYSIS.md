...
| DAT-05 | Un asociado no puede repetirse según su identificador único | PARCIAL (inconsistencia corregida; identificador de negocio sigue abierto) | Antes de esta sesión: `AssociateRequest` (alta/edición manual) no validaba correos duplicados, mientras que `AssociateImportService::parse()` sí lo hacía — un mismo correo podía cargarse dos veces a mano pero no por Excel. **Corregido:** se agregó `Rule::unique('associates', 'email')->ignore(...)` a `AssociateRequest`, con tests (`test_duplicate_email_is_rejected_on_manual_create`, `test_multiple_associates_without_an_email_do_not_collide`, `test_editing_an_associate_keeping_its_own_email_is_not_a_duplicate`) | Esto **no** resuelve la pregunta 12 (¿cuál es el identificador de negocio real — correo, RUC, nombre?) — solo hace que ambos caminos de alta apliquen la misma regla que ya existía en uno de los dos. No se agregó unicidad de `name`, ni se hizo `email` obligatorio, ni se introdujo RUC — eso sigue `REQUIERE VALIDACIÓN DEL CLIENTE` |
...
| INV-08 | ¿Se puede corregir/anular un pago ya registrado? | FALTANTE | `PaymentController` solo tiene `store()`. No existe `update()`, `destroy()`, ni ningún mecanismo de reversa/nota de crédito | Ver pregunta 7 — es el hallazgo de mayor impacto operativo de todo este análisis: un error de tipeo en un monto de pago hoy es **irreversible** desde la aplicación |
...
**Requiere decisión del cliente antes de tocar código** (ver `docs/OPEN_BUSINESS_DECISIONS.md` para las 20 preguntas completas): identificador de negocio del asociado más allá del correo — ej. RUC (pregunta 12), regla de inactivación de asociados (ASO-02), si una factura vencida puede recibir pagos (INV-03, aunque el comportamiento actual ya es razonable) y si las facturas/pagos pueden corregirse o anularse (INV-04/05/08 — este último es el de mayor impacto operativo real).

**Riesgos:**
1. **Alto impacto operativo, bajo riesgo técnico:** un pago mal ingresado hoy no se puede corregir ni desde la UI ni sin acceso directo a la base de datos (INV-08). Si esto no es aceptable para la operación diaria de la Cámara, es la brecha de mayor prioridad de negocio de todo este análisis.
2. **Riesgo bajo (mitigado en esta sesión):** la inconsistencia de duplicados entre alta manual e importación (DAT-05) ya no existe; sigue abierta la pregunta de fondo sobre cuál es el identificador de negocio real del asociado (pregunta 12).
