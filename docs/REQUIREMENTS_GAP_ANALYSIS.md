...
| XLS-04 | Importación de **facturas** y **pagos** desde Excel | **RESUELTO (2026-09-14)** | `InvoiceImportController`/`InvoiceImportService` y `PaymentImportController`/`PaymentImportService` — mismo flujo de dos pasos (previsualizar → confirmar) que el importador de asociados. Ninguno de los dos crea el asociado ni la factura si no existe de antemano; el de pagos reutiliza `PaymentService::register()` fila por fila para que `paid_total`/`status` queden exactamente igual de consistentes que un registro manual | Autorizado explícitamente por el cliente en esta sesión — ya no es un cambio de alcance pendiente |
