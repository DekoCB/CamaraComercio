...
## 12. Adenda — 2026-08-19 (2): sidebar sin parpadeo al navegar, wizard de facturación en modal, gráfico sin líneas blancas

### 12.1 El sidebar colapsado "parpadeaba" al cambiar de módulo

Reportado por el usuario: con el sidebar colapsado, cada navegación (esta es una MPA server-rendered — cada clic en el menú recarga la página completa) mostraba el sidebar expandido por una fracción de segundo y luego se colapsaba de golpe. Causa: `shell.classList.add('is-collapsed')` corría en `app.js`, cargado al final de `<body>` — es decir, el sidebar ya había pintado en su estado expandido por defecto antes de que ese script corriera, y como `.sidebar` tiene `transition: width`, el cambio a colapsado se **animaba** visiblemente después de la carga en cada página. Mismo tipo de causa que el parpadeo de tema ya resuelto en la sección 11.1.

Corrección: el estado colapsado ahora se aplica **antes de la primera pintura**, igual que el tema — el script inline en `<head>` de `layouts/app.blade.php` lee `localStorage.getItem('cc_sidebar_collapsed')` y, si corresponde, agrega la clase `sidebar-collapsed` a `<html>` (no a `.app-shell`, que todavía no existe en el DOM en ese punto). Todos los selectores CSS que dependían de `.app-shell.is-collapsed` pasaron a `html.sidebar-collapsed .app-shell` (selector descendiente, funciona independientemente del momento en que se parseó `.app-shell`). Verificado con Playwright: muestreo del ancho del sidebar en cada frame durante los 400ms posteriores a una navegación — se mantiene en 72px de forma constante, ni un solo frame en 260px.

### 12.2 Generar facturación del mes como modal

A pedido del usuario, el wizard de generación de facturas se suma a los formularios que se abren como overlay (sección 10.2) en vez de navegar a `/invoices/generate`. Mismo mecanismo (partial `invoices/_generate.blade.php` + `InvoiceController@create` con rama `$request->ajax()`), con una diferencia importante: el wizard tiene JS propio no trivial (navegación entre 4 pasos, validación por paso, resumen antes de confirmar) que antes vivía en un bloque `@push('scripts')` dentro de la vista de página completa — eso no sirve para el fragmento AJAX, que no tiene layout ni `@stack` que lo reciba. Se trasladó esa lógica a `app.js` como `initInvoiceWizard(root)`, con todas las consultas escopadas a `root` en vez de `document.getElementById` directo, invocada tanto contra `document` (carga de página completa, fallback sin JS) como contra `formModalBody` (dentro del modal, justo después de inyectar el HTML) — mismo patrón que `enhanceSelects`/`enhanceDateInputs`.

**Bug real encontrado y corregido durante la verificación:** este formulario tiene `data-confirm` (HU-20: "¿Generar facturas?" antes de una acción masiva irreversible). El sistema de confirmación existente y el nuevo sistema de modal-formulario-por-AJAX son dos mecanismos independientes que nunca habían coexistido en el mismo `<form>`; al combinarlos, el listener de submit de `wireModalForm` (atado directamente al `<form>`) se ejecutaba *antes* que el listener delegado de confirmación (atado a `document`, se dispara después por el orden de burbujeo) — el envío por AJAX arrancaba inmediatamente, y el diálogo de confirmación aparecía *después*, ya con la petición en curso. Se resolvió exponiendo la función de envío AJAX como `form.__submitViaAjax`, haciendo que: (a) el listener de `wireModalForm` no haga nada si el formulario tiene `data-confirm` sin confirmar todavía (deja que el flujo de confirmación existente se encargue), y (b) el botón "Confirmar" del diálogo llame a `form.__submitViaAjax()` en vez de a un `submit()` nativo cuando esa función existe. Un segundo bug relacionado, encontrado en la misma verificación: el diálogo de confirmación y el modal de formulario comparten las mismas clases `.modal-backdrop-custom`/`.modal-dialog-custom` y por lo tanto el mismo z-index — al abrirse uno sobre el otro, el que aparece después en el HTML (el modal de formulario) tapaba visualmente al de confirmación aunque ambos estuvieran "abiertos" en el DOM. Corregido con un z-index específico y mayor para `#confirmModalBackdrop`/`#confirmModalDialog`, para que la confirmación siempre quede por encima de cualquier modal de formulario que la haya disparado.

### 12.3 Gráfico de cobranza mensual sin líneas de cuadrícula

Las líneas horizontales del eje Y (`grid: { color: '#F1F5F9' }`, un gris casi blanco pensado solo para modo claro) se veían como rayas blancas duras sobre el fondo oscuro del `chart-card` en modo oscuro. En vez de detectar el tema en JS para elegir un color por modo, se quitó la cuadrícula del eje Y por completo (`grid: { display: false }`, igual que ya tenía el eje X) — más simple, funciona igual de bien en ambos temas, y es un estilo de gráfico limpio ya usado en el eje X del mismo chart.
