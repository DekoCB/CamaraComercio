...
| — | EP-08 | Configuración de perfil de usuario | Media | — | **DONE** (2026-08-19) | `ProfileController` + `ProfileUpdateRequest`, autoservicio (cualquier usuario autenticado edita su propio perfil, sin permiso adicional): foto (`avatar_path`, `storage/app/public/avatars`, servida vía `php artisan storage:link`), nombre, correo (único, ignorando al propio usuario) y contraseña (opcional — requiere la contraseña actual solo si se está cambiando, vía la regla nativa `current_password`). Se abre como modal desde el dropdown del topbar (`<x-avatar>`, reutilizado también ahí en vez de las iniciales calculadas inline que había antes). 13 tests nuevos; bug real encontrado en la verificación con navegador real: un formulario HTML siempre envía todos sus campos (aunque el usuario no los haya tocado), a diferencia de una petición de test con una clave simplemente omitida — la regla `current_password` no tenía `nullable` y rechazaba cualquier guardado que no cambiara la contraseña, porque comparaba un string vacío contra el hash real. |
...
## Criterios de aceptación — Importación de facturas y pagos desde Excel (2026-09-14)
