...
### 10.19 Configuración de perfil de usuario — 2026-08-19

A pedido del usuario: una pantalla de autoservicio donde cualquier usuario autenticado edita su propio perfil — por ahora, foto, nombre, correo y contraseña (backlog fila EP-08 "Configuración de perfil de usuario", sin HU numerada porque no estaba en la documentación funcional original).

- **Modelo:** migración `avatar_path` (string, nullable) en `users`; `Storage::disk('public')` + `php artisan storage:link` (no existía el symlink todavía, se creó en esta sesión) para servir la foto. `User::initials()` y `User::avatarUrl()` centralizan lo que antes era una expresión `Str::of(...)` calculada inline solo en el topbar.
- **Componente `<x-avatar :user="...">`:** primer componente Blade de avatar del sistema — muestra la foto si existe, si no las iniciales (mismo fallback visual de siempre). Reemplaza la expresión inline del topbar y se reutiliza en el propio formulario de perfil.
- **Backend:** `ProfileController` (`edit`/`update`) sigue exactamente el patrón de modal-por-AJAX ya establecido (fragmento `profile._form` vs. página completa `profile.edit` según `$request->ajax()`); `ProfileUpdateRequest::authorize()` devuelve `true` siempre — es autoservicio, no depende de ningún permiso de rol. La contraseña es opcional: si no se toca, no cambia; si se toca, exige la contraseña actual (regla nativa `current_password`) y confirmación.
- **Bug real encontrado con navegador real, no con los tests:** los 12 tests iniciales pasaban en verde pero la app fallaba en el navegador — un `<form>` HTML real envía **todos** sus campos siempre (`current_password=""` incluido, aunque el usuario nunca lo haya tocado), mientras que `$this->put('/profile', [...])` en un test con una clave omitida es indistinguible de "campo ausente". La regla `current_password` no tenía `nullable`, así que comparaba ese string vacío contra el hash real y rechazaba **cualquier** guardado del perfil que no cambiara la contraseña. Se agregó `nullable` a la regla y un test nuevo que replica la forma real de un envío de navegador (todas las claves presentes, contraseña vacía) para que no vuelva a pasar inadvertido. Exactamente el tipo de bug que la verificación con Playwright en este proyecto ya había atrapado antes (parpadeo de sidebar, calendario recortado) — confirma por qué "pasa el test" y "funciona en el navegador" no son lo mismo.
- **Verificación:** 13/13 tests de `ProfileTest` en verde (84/84 en total); Playwright con una foto real subida vía `set_input_files` — confirmado que se guarda, se sirve por la URL del disco `public`, y se refleja tanto en la vista previa del formulario como en el avatar del topbar; cambio de contraseña verificado cerrando sesión y volviendo a entrar con la contraseña nueva; modo oscuro y mobile revisados sin overflow ni fugas de estilo.

### 10.20 Auditoría de aptitud para producción — 2026-08-19

A pedido explícito del usuario ("IMPLEMENTACIÓN DE REQUERIMIENTOS FALTANTES", 30 secciones): auditoría completa del repositorio contra los requerimientos de producción que las 23 HU originales no cubrían (integridad de datos, seguridad, protección de datos, auditoría, backups, QA/UAT, deployment, capacitación, gestión de cambios), sin modificar ninguna HU-01 a HU-23.

**Metodología:** lectura directa del código fuente (migraciones, modelos, servicios, Form Requests, rutas, configuración) — no de documentación previa — para separar cada hallazgo en Definida / Inferible / No definida, exactamente como pedía la regla principal del encargo.

**16 documentos entregados** (los 4 que el propio prompt marcaba como "primer paso" — `REQUIREMENTS_GAP_ANALYSIS.md`, `OPEN_BUSINESS_DECISIONS.md`, `DATA_SCHEMA.md`, `BUSINESS_RULES.md` — más los 12 restantes de la sección "resultado final", incluido este mismo documento): `TECHNICAL_SPECIFICATION.md`, `EXCEL_MIGRATION_SPECIFICATION.md`, `DATA_PROTECTION.md`, `BACKUP_AND_RECOVERY.md`, `QA_PLAN.md`, `UAT_PLAN.md` (+ `UAT_SEED_DATA.md`, no pedido por nombre pero referenciado desde el plan de UAT), `ACCEPTANCE_CRITERIA.md`, `DEFINITION_OF_DONE.md`, `DEPLOYMENT.md`, `USER_MANUAL.md`, `CHANGE_MANAGEMENT.md`.

**Hallazgo principal:** el sistema ya tenía, verificado con evidencia de código (no solo con la documentación previa), casi todo lo que un auditor de producción esperaría — RBAC server-side real, hash de contraseñas, rate limiting en login, protección CSRF/XSS/SQLi, prevención de sobrepago con `lockForUpdate()` transaccional, constraint de unicidad factura+período a nivel de base de datos, y auditoría de las operaciones críticas. La auditoría no encontró un sistema desprotegido — encontró una lista concreta de 20 decisiones de negocio nunca formalizadas (`docs/OPEN_BUSINESS_DECISIONS.md`) y algunas piezas de documentación/operación que nunca se habían escrito porque nadie las había pedido todavía.

**Dos correcciones de código, ambas categoría A/B (sin depender de ninguna decisión pendiente):**
1. Se centralizó la fórmula de saldo (`amount - paid_total`) en `Invoice::BALANCE_SQL`, usada por `ReportService` y `DashboardService` en vez de repetir el fragmento SQL suelto — la sección 8 del encargo pedía explícitamente "no duplicar reglas de cálculo".
2. Se cerró una inconsistencia real: el alta manual de asociados no rechazaba correos duplicados, mientras que la importación por Excel sí — ahora ambos caminos aplican la misma regla (`AssociateRequest` con `Rule::unique`), con 3 tests nuevos.

**El hallazgo de mayor impacto operativo no se implementó, a propósito:** un pago o una factura, una vez registrados, no se pueden corregir ni anular desde la aplicación — es un comportamiento existente desde los sprints originales, nunca antes señalado como una limitación explícita. No se agregó esa funcionalidad porque decidir *cómo* debería funcionar (¿anulación con motivo? ¿nota de crédito? ¿quién puede hacerlo?) es exactamente el tipo de regla de negocio crítica que este encargo prohíbe inventar — queda documentado como la pregunta 8 de `docs/OPEN_BUSINESS_DECISIONS.md`.

**Datos de prueba:** se agregó `database/seeders/UatDemoDataSeeder.php` (idempotente, `updateOrCreate`) porque el seeder existente dejaba el sistema sin ninguna factura ni pago tras un `migrate --seed` limpio — ahora cubre los 4 estados de factura (PENDIENTE/PARCIAL/PAGADA/VENCIDA) y un asociado inactivo, necesarios para ejecutar `docs/UAT_PLAN.md` de forma representativa.

**Verificación:** 87/87 tests en verde (3 nuevos de la corrección de duplicados), Pint limpio. No se tocó ninguna HU, ninguna arquitectura, ni se implementó nada que dependiera de una de las 20 preguntas abiertas.

### 10.21 Próximo paso

Con los cuatro sprints funcionales, el rediseño UI/UX, las mejoras de UX de formularios/calendarios/tema/sidebar, la configuración de perfil y esta auditoría de producción completos, lo pendiente es: (1) obtener respuesta del cliente a las 20 preguntas de `docs/OPEN_BUSINESS_DECISIONS.md` — en particular la 8 (corrección/anulación de pagos) y la 12 (identificador de negocio del asociado), que son las de mayor impacto; (2) definir el proveedor SMTP real para producción; (3) definir el entorno de producción real (`docs/DEPLOYMENT.md` sección 1) para poder configurar backups reales (`docs/BACKUP_AND_RECOVERY.md`) en vez de solo la propuesta; (4) ejecutar `docs/UAT_PLAN.md` con un usuario real de la Cámara; (5) cualquier extensión fuera del alcance original del MVP, que requeriría aprobación explícita según `docs/CHANGE_MANAGEMENT.md`.

### 10.22 Anulación de pagos e identificador RUC del asociado — 2026-08-28

A pedido explícito del usuario ("procede con la implementación de la pregunta 8 y la 12"), se implementaron las dos decisiones de negocio de mayor impacto identificadas por la auditoría 10.20: corrección/anulación de pagos y el identificador único del asociado.

**Corrección documental previa:** la pregunta de anulación de pagos, referida informalmente como "pregunta 8" en el resumen de la auditoría anterior, nunca tuvo un número propio en `docs/OPEN_BUSINESS_DECISIONS.md` — la pregunta 8 real de ese documento es "¿puede una factura tener múltiples pagos?" (ya resuelta). El tema solo existía como hallazgo `INV-08` en `docs/REQUIREMENTS_GAP_ANALYSIS.md`, con una cita cruzada equivocada ("Ver pregunta 7", que trata sobre eliminar facturas). Se agregó como pregunta 21 y se corrigió la cita.

**Anulación de pagos (pregunta 21):** un pago nunca se edita ni se elimina — se anula, con motivo obligatorio, preservando el registro original (misma política de "sin borrado físico" que rige el resto del sistema). `payments.voided_at`/`voided_by`/`void_reason` (migración `2026_08_20_100011`); `PaymentService::void()` transaccional con `lockForUpdate()`, recalcula `paid_total` sumando desde cero los pagos activos; permiso nuevo `payments.void` (solo Administrador por defecto); UI de anulación en `invoices/show.blade.php` y `payments/index.blade.php`; `ReportService`/`DashboardService` excluyen pagos anulados de los totales cobrados vía `Payment::scopeActive()`. 7 tests nuevos.
...
### 10.26 Parte 1 de la retroalimentación del cliente (demo grabada, minutos 0–10) — 2026-09-17

El cliente dejó grabado un video de una demo con retroalimentación; el usuario lo transcribió en dos actas (`acta1.txt`, 0:00–10:00; `acta2.txt`, 10:00 en adelante) y pidió primero un análisis de qué faltaba de los primeros 10 minutos y qué implicaría el resto, sin tocar código — entregado como informe estructurado (✅ hecho / ⚠️ parcial / ❌ falta / 🔧 solo configuración / 🗓️ bloqueado por el cliente). Luego, a pedido explícito ("Dale prioridad a la parte 1, implementa lo que aun no hay de ahi"), se implementó todo lo identificado como faltante en la parte 1; la parte 2 (documentos escaneados, portal de autoservicio del asociado, seguimiento de beneficios/auditorio, integración contable) queda explícitamente fuera de este alcance hasta nueva instrucción.

**Cuatro roles nuevos** (`acta2.txt [10:41]`, citada desde la parte 1 por describir la jerarquía de accesos que el cliente pidió desde la demo): **Gerencia** (mismo alcance operativo que Encargado de Cobranzas, sin Administración), **Logística** y **Marketing** (solo lectura de Dashboard + Asociados, sin facturación/pagos/administración), **Gestión de Asociados** (alta/edición/importación del padrón + Cartera de solo lectura). El acta no especifica en detalle los permisos positivos de Logística/Marketing, solo lo que deben tener prohibido — se les dio el mínimo razonable (Dashboard + Asociados) documentado en el propio seeder como ajustable sin tocar código desde Administración → Roles. No se crearon cuentas de demo para estos 4 roles.

**N° de operación en los pagos:** columna `payments.operation_number` (nullable, `payments/{migración}`), campo opcional en ambos formularios de registro de pago (rápido y desde la factura), mostrado como sub-línea bajo el método de pago en las tres tablas de historial de pagos (factura, Pagos → Movimientos, Cartera → Historial de pagos). No se tocó el importador de Excel — no fue parte de lo identificado como faltante.

**Editar y anular facturas** — el hallazgo de mayor impacto de la parte 1: no existía ninguna manera de corregir una factura mal generada. Se implementó reutilizando exactamente el patrón ya aprobado para pagos (`PaymentService::void()`, pregunta 21 de `OPEN_BUSINESS_DECISIONS.md`) en vez de inventar un mecanismo nuevo: `invoices.voided_at`/`voided_by`/`void_reason`, editar y anular solo permitidos con `paid_total == 0` (`InvoiceService::assertEditable()`), nunca un `DELETE`. Nuevo estado `Invoice::STATUS_ANULADA`, calculado igual que `VENCIDA` (nunca persistido, ver `effectiveStatus()`), con su propio badge visual (tachado, neutro). Permisos nuevos `billing.edit`/`billing.void`.

Anular una factura la excluye de todo lo que antes la contaba como "facturado" o "pendiente de cobro" — no solo de "pendiente", ya que una factura anulada nunca debió existir para efectos contables. Esto tocó `DashboardService`, `ReportService`, `PortfolioService` e `InvoiceStatsService` en cada punto donde sumaban `amount`/`balance` sobre facturas, además de bloquear registrar pagos sobre una factura anulada (`PaymentService::register()`) y sacarla de los selectores de "registrar pago". Bug real encontrado por la suite de tests al aplicar esto: `Payment::scopeActive()`/`scopeVoided()` sin calificar (`voided_at` a secas) se volvieron ambiguos en cuanto `invoices` también tuvo su propia columna `voided_at` y una consulta hacía `JOIN` entre ambas tablas (`ReportService::collections()`) — se corrigió calificando `payments.voided_at` explícitamente.

**Reporte de cobranza con rango de fechas:** `ReportService::collections()` seguía aceptando solo un mes calendario. Ahora acepta opcionalmente `date_from`/`date_to`: "cobrado" usa las fechas exactas, "facturado" suma cada período de factura que el rango toca — comportamiento 100% retrocompatible (un solo período sigue funcionando idéntico a antes, sin `date_from`/`date_to`).

**Reporte nuevo — productividad por cobrador:** `ReportService::collectorProductivity()`, agrupa pagos por `registered_by` (mismo selector de período/rango que el de cobranza), con total, cantidad, promedio y % del total por usuario; pagos sin `registered_by` (datos legado) se agrupan bajo "Sin asignar" en vez de perderse del total. Nueva vista `reports/collectors.blade.php` + exportación Excel/PDF, enlazada desde `reports/index.blade.php`.

**Exportación de Cartera:** ni `PortfolioController::index()` ni `::debtors()` tenían exportación — solo Reportes la tenía. Se agregó Excel/PDF a ambas, respetando los filtros activos (`debtSummaryForExport()`/`debtorsForExport()` en `PortfolioService`, mismas consultas que las paginadas sin paginar), gateado por `reports.export` además de `portfolio.view` (mismo criterio que Reportes: ver en pantalla no implica poder extraer el dato). Como esas dos vistas usan filtrado en vivo por AJAX (`data-live-filter`), los botones de exportación no podían simplemente leer el query string del primer render — se agregó `updateExportLinks()` en `app.js` para que los enlaces de exportación se actualicen con los filtros activos cada vez que el filtro en vivo recarga la tabla.

**Verificación:** 184/184 tests en verde (14 nuevos: 9 de anulación/edición de facturas, 2 de rango de fechas y productividad en `ReportTest`, 3 de exportación de Cartera en `PortfolioTest`, 2 de N° de operación en `PaymentMethodTest`, 1 de notificación de factura anulada), Pint limpio. Las 4 roles se corrieron localmente vía `db:seed --class=RolesPermissionsModulesSeeder` y se confirmaron creadas. (Nota: la suite tiene un test preexistente — `PaymentImportTest::test_row_with_amount_greater_than_the_invoice_balance_is_flagged` — que falla intermitentemente en corrida completa pero pasa siempre aislado; reproducido antes de tocar código de este addendum, no relacionado con estos cambios, no investigado a fondo por estar fuera del alcance de la parte 1.)

Pendiente para la parte 2, explícitamente no iniciado: carga de documentos escaneados + generación de PDF, portal de autoservicio del asociado, seguimiento de beneficios/uso del auditorio, integración de asientos contables ("contasis" — bloqueado en que el cliente envíe un video/ejemplo).

### 10.27 Parte 2 de la retroalimentación del cliente (demo grabada, minutos 10–fin) — 2026-09-18

Continuación directa de la sección 10.26. El commit de la parte 1 (`9731398`) ya se había subido a GitHub con `git push` (confirmado con el usuario que nada se había desplegado a Hostinger). A pedido explícito ("Ahora si continua con la parte 2"), y dado que el análisis detallado de `acta2.txt` había quedado solo en la conversación anterior (no en un documento), se releyeron ambas actas desde la carpeta de Descargas del usuario antes de tocar código, siguiendo la misma disciplina de "no inventar alcance en silencio" de toda la sesión.

**Alcance accionable ahora, según lo que el propio cliente marcó como listo vs. bloqueado:**
- ✅ Documentación escaneada por asociado con generación de PDF (`acta2.txt [11:39]`).
- ✅ Carga de fotos/logos con un límite mayor (`acta2.txt [11:39]`) — el acta pedía literalmente "menor a 1 GB", pero 1 GB por archivo es poco razonable en el hosting compartido de Hostinger (riesgo real de agotar la cuota de disco); se consultó al usuario vía pregunta directa y se optó por **10 MB**, muy por debajo de lo literal del acta pero muy por encima del límite de 2 MB que ya existía.
- ✅ Control de beneficios de los asociados, ej. uso anual del auditorio (`acta2.txt [~15:30]`).
- 🗓️ **No iniciado, a propósito:** portal de autoservicio del asociado — el cliente mismo lo enmarcó como "a futuro" (`acta2.txt [11:39]`), no un pedido para ahora.
- 🗓️ **No iniciado, bloqueado:** integración de asientos contables ("contasis") — el cliente debe enviar primero un video/ejemplo de su proceso actual; no hay nada que se pueda diseñar sin eso.
- 🗓️ **No accionable, bloqueado en datos:** poblar Reportes → Deuda pendiente con cifras reales — el reporte ya existe y funciona (Sprint 3, HU-14); lo que falta es que el cliente envíe su Excel de deudas pendientes, no código.

**Documentación escaneada:** tabla `associate_documents` (tipo cerrado a Título de propiedad/Licencia/Convenio/Otro, nombre original, ruta del PDF ya normalizado, tamaño, quién lo subió), `AssociateDocumentService::upload()`. Como una aplicación web no puede manejar un escáner físico, "generar un PDF digitalizado" se interpretó como: si el archivo subido ya es un PDF se guarda tal cual; si es una foto/imagen (celular o app de un escáner), se envuelve en un PDF de una página con Dompdf (la misma librería que ya usa `ExportService` para los reportes) — así el resultado guardado es siempre un único PDF, sin depender de qué formato subió el usuario. Subir/eliminar documentos requiere `associates.manage` (mismo permiso que el resto de la gestión del padrón); verlos es tan abierto como ya lo es ver la ficha del asociado, sin permiso nuevo.

**Límite de imagen del asociado:** `AssociateRequest` pasó de 2 MB a 10 MB (logo/foto del asociado, campo `image` que ya existía desde antes de esta sesión) — decisión de tamaño de archivo confirmada explícitamente por el usuario antes de tocar código (ver arriba). El avatar de perfil del usuario del sistema (`ProfileUpdateRequest`, un campo distinto y sin relación con este pedido del cliente) se dejó en 2 MB, sin cambios.

**Control de beneficios:** dos tablas nuevas, `benefits` (catálogo — nombre, cupo anual, activo/inactivo) y `benefit_usages` (asociado, beneficio, fecha de uso, notas, quién lo registró). El acta solo describió un beneficio concreto ("uso gratuito anual del auditorio"), pero lo enmarcó en general ("control de beneficios de los asociados"), así que se modeló como catálogo en vez de un campo fijo — un segundo beneficio es una fila nueva (`BenefitsSeeder`), no una migración. `BenefitService::register()` calcula el uso ya registrado en el año calendario de la fecha dada y rechaza un registro que exceda el cupo anual del beneficio, con el mismo mensaje de error mostrado en el formulario. No hay pantalla de administración del catálogo de beneficios todavía — el pedido concreto era "saber si ya usaron o no el beneficio", no administrar tipos de beneficio; se documentó como una limitación consciente, no un olvido.

**Dónde vive todo:** una nueva sección "Documentos" y otra "Beneficios" al final de `associates/show.blade.php` — no una pantalla aparte, siguiendo el mismo patrón de tarjetas apiladas que ya tenía esa página (Datos generales, Direcciones, Representantes, Observaciones...).

**Verificación:** 197/197 tests en verde (13 nuevos: 7 de documentos en `AssociateDocumentTest`, incluida la conversión real imagen→PDF vía Dompdf verificada leyendo la cabecera `%PDF` del archivo resultante; 6 de beneficios en `BenefitUsageTest`, incluido el reinicio del cupo en el año calendario siguiente), Pint limpio. Migraciones y el seed de `BenefitsSeeder` corridos localmente. (Nota: la suite volvió a mostrar el mismo patrón de test intermitente ya señalado en la sección 10.26 — esta vez en distintas aserciones de `InvoiceImportTest`, siempre en corrida completa, siempre en verde aislado; no relacionado con el código de esta sección, sigue sin investigarse a fondo por estar fuera de alcance.)

No se desplegó nada a Hostinger ni se hizo `git push` de este trabajo — pendiente de confirmación explícita, igual que la parte 1.
