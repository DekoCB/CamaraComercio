...
Esto se puso a prueba en Sprint 2: la generación masiva de facturas (HU-06) y el registro de pagos (HU-08/HU-09) sí tenían lógica demasiado compleja para vivir en un controlador o en el modelo — manejo de errores por ítem en un lote, bloqueo transaccional para evitar condiciones de carrera. En vez de reintroducir capas Domain/Application completas, esa lógica se extrajo a clases de servicio puntuales en `app/Services/` (`InvoiceGenerationService`, `PaymentService`), inyectadas en el controlador correspondiente. La máquina de estados de la factura (`PENDIENTE`/`PARCIAL`/`PAGADA`/`VENCIDA`) sí quedó como métodos del modelo `Invoice` (`effectiveStatus()`, `isOverdue()`, scope `overdue()`) porque es lectura derivada de sus propias columnas, no un caso de uso con pasos ni efectos secundarios — ver `docs/DATA_MODEL.md` para el razonamiento de por qué `VENCIDA` nunca se persiste.

Sprint 3 confirmó el mismo patrón para lecturas agregadas, no solo para escrituras: `PortfolioService` (cartera, HU-10/11/12) y `ReportService` (reportes, HU-13/14) encapsulan consultas con `withSum`/`withCount`/`whereHas`/`GROUP BY` que serían ilegibles repetidas en cada controlador, y `ExportService` centraliza la generación de Excel/PDF para que cada reporte no reimplemente su propio cableado de PhpSpreadsheet/Dompdf. Ninguno de los tres es "lógica de negocio" en el sentido de reglas que protejan invariantes — son ensamblado de datos de solo lectura —, pero igual ameritan una clase propia por la misma razón que `InvoiceGenerationService`/`PaymentService`: son demasiado largos y con demasiadas partes móviles para vivir en un método de controlador sin perder legibilidad.

Sprint 3 también dejó una lección de portabilidad concreta: una consulta con `havingRaw()` sobre alias de `withSum()` funcionaba contra MySQL pero rompía contra el SQLite de los tests ("HAVING clause on a non-aggregate query" — SQLite exige que un HAVING sin GROUP BY acompañe una función agregada real en el nivel superior de la consulta; MySQL es más permisivo). La solución no fue añadir una rama condicional por driver, sino replantear la condición usando el invariante de negocio ya garantizado por la máquina de estados (`whereHas('invoices', fn ($q) => $q->where('status', '!=', 'PAGADA'))` en vez de comparar sumas), lo cual además quedó más legible. Cuando SQL crudo sí fue necesario (la distribución de deuda por estado en `ReportService::pendingDebt()`), "hoy" se pasa como parámetro ligado en vez de usar `CURDATE()` (función específica de MySQL) — ver el comentario en el propio servicio. Regla general: preferir expresar condiciones vía el invariante de negocio antes que vía SQL agregado cuando ambos son posibles; cuando haga falta SQL crudo, evitar funciones específicas del motor si el mismo código debe correr contra SQLite en los tests.

Sprint 4 añadió `AssociateImportService`, con la misma justificación (un flujo de tres pasos — cargar, previsualizar, confirmar — es demasiado para un método de controlador) y una regla de seguridad que vale la pena nombrar explícitamente: **el paso de confirmación nunca confía en datos que el navegador reenvía sobre lo que se previsualizó.** El archivo subido se guarda en `storage/app/imports/` con un nombre aleatorio al momento de la vista previa, y `AssociateImportController::confirm()` vuelve a leer y re-validar ese mismo archivo desde disco antes de crear cualquier registro — así, alguien que intente manipular el formulario de confirmación para colar una fila que nunca pasó la validación de la vista previa simplemente no tiene nada que manipular (el servidor ignora cualquier dato de fila en el POST y solo mira el archivo). Es el mismo principio que ya aplicaba `PaymentService` (recalcular el saldo desde la base de datos con `lockForUpdate()`, no confiar en un monto que venga del cliente): la fuente de verdad para cualquier operación que escribe datos es siempre el servidor, nunca lo último que el formulario mostró en pantalla.
